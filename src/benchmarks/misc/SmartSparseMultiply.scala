...
object SmartSparseMultiply {
	...
	*/
	...
	def matmult(y: Array[AffineFloat], vall: Array[AffineFloat], row: Array[Int],
		col: Array[Int], x: Array[AffineFloat], NUM_ITERATIONS: Int) = {
  ...
  def initialize(N: Int, nz: Int): (Array[AffineFloat], Array[Int], Array[Int])  = {
    ...
    val nr: Int = nz/N 		// average number of nonzeros per row
		...
		val vall: Array[AffineFloat] = arandomVector(anz)
  ...
  /*
  public static AffineFloat measureSparseMatmult(int N, int nz, 
			AffineFloat min_time, Random R)
	{
		// initialize vector multipliers and storage for result
		// y = A*y;

		AffineFloat x[] = RandomVector(N, R);
		AffineFloat y[] = new AffineFloat[N];
		...
		AffineFloat val[] = RandomVector(anz, R);
		int col[] = new int[anz];
		int row[] = new int[N+1];
