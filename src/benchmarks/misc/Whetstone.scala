...
object Whetstone {
  ...
  private val e1: Array[Double] = Array.fill(4){0.0}
  private var routine = 0
  private val x100 = 100
  
  private var t =  0.49999975
  private val t0 = t        
  private val t1 = 0.50000025
	private val t2 = 2.0
  ...
  private def pa(e: Array[Double], t: Double, t2: Double) {
    var j = 0L
    for(j <- 0 until 6) {
      e1(0) = (e1(0) + e1(1) + e1(2) - e1(3)) * t
		  e1(1) = (e1(0) + e1(1) - e1(2) + e1(3)) * t
  ...
  private def p3(t: Double, t1: Double, t2: Double) {
    x = y
    y = z
    x = t * (x + y)
    y = t1 * (x + y)
  ...
  def po(e1: Array[Double], j: Int, k: Int, l: Int) {
    e1(j) = e1(k)
    e1(k) = e1(l)
    e1(l) = e1(j)
    return
