...
object AffineFBench {
  ...
  val clear_aperture = AffineFloat(4.0)
  var aberr_lspher = AffineFloat(0.0)
  var aberr_osc = AffineFloat(0.0)
  var aberr_lchrom = AffineFloat(0.0)
  var max_lspher = AffineFloat(0.0)
  var max_osc = AffineFloat(0.0)
  var max_lchrom = AffineFloat(0.0)

  var radius_of_curvature = AffineFloat(0.0)
  var object_distance = AffineFloat(0.0)
  var ray_height = AffineFloat(0.0)
  var axis_slope_angle = AffineFloat(0.0)
  var from_index = AffineFloat(0.0)
  var to_index = AffineFloat(0.0)

  val spectral_line = Array.fill(9){AffineFloat(0.0)}
  val s = Array.fill(max_surfaces, 5){AffineFloat(0.0)}
  val testcase: Array[Array[AffineFloat]] = Array(
        Array(27.05, 1.5137, 63.6, 0.52),
        Array(-16.68, 1, 0, 0.138),
        Array(-16.68, 1.6164, 36.7, 0.38),
        Array(-78.1, 1, 0, 0));

  var od_sa = Array.fill(2, 2){AffineFloat(0.0)}
  ...
  */
  ...
  def cot(x: AffineFloat): AffineFloat = 1.0 / tan(x)
  ...
  def trace_line(line: Int, ray_h: AffineFloat) = {
    var i = 0
  ...
  def transit_surface: Unit = {
    var iang,     /* Incidence angle */
        ...
        old_axis_slope_angle, sagitta = AffineFloat(0.0)
  ...
  def run(niter: Int): Long = {
    ...
    //Date    Now;
    var  time = 0L
    var  od_cline, od_fline = AffineFloat(0.0)
    var runtime = AffineFloat(0.0)
    ...
    /*printf("\n\nResults\n\n")
    println("Marginal ray      " +od_sa(0)(0)+ "  "+od_sa(0)(1)+" \n")
    println("Paraxial ray      "+od_sa(1)(0)+"  "+od_sa(1)(1)+" \n")
    println("Longitudinal spherical aberration:      "+aberr_lspher+" \n")
    println("    (Maximum permissible):              "+ max_lspher+" \n")
    println("Offense against sine condition (coma):  "+aberr_osc+" \n")
    println("    (Maximum permissible):              "+max_osc+" \n")
    println("Axial chromatic aberration:             "+aberr_lchrom+" \n")
    println("    (Maximum permissible):              "+max_lchrom+" \n")
    println("Finished, clock =  %d\n", time)*/
  ...
  //@return some number corresponding of accuracy of computations
  def runNoTiming(niter: Int)= () => {
    ...
    val res = od_sa(0)(0) //+ od_sa(0)(1) + od_sa(1)(1) + od_sa(1)(0)
