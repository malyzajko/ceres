...
import smartfloat.{AffineFloat, IntervalFloat, SmartFloat}

object NBody {
  
  def doubleSimulate(N: Int, dt: Double): Double = {
    var n = N
    JovianSystem.reset
    while (n > 0) { JovianSystem.advance(dt); n -= 1; }
      //println(n + " " + JovianSystem.energy)}
    JovianSystem.energy
  }

  def affineSimulate(N: Int, dt: Double): AffineFloat = {
    var n = N
    AffineJovianSystem.reset
    while (n > 0) { AffineJovianSystem.advance(dt); n -= 1; }
      //println(n + " " + AffineJovianSystem.energy)}
    AffineJovianSystem.energy
  }

  def intervalSimulate(N: Int, dt: Double): IntervalFloat = {
    var n = N
    IntervalJovianSystem.reset
    while (n > 0) { IntervalJovianSystem.advance(dt); n -= 1; }
     // println(n + " " + IntervalJovianSystem.energy)}
    IntervalJovianSystem.energy
  }

  def smartSimulate(N: Int, dt: Double): SmartFloat = {
    var n = N
...
object JovianSystem {
  ...
  protected var bodies: Array[Body] = initialValues
  ...
  def advance(dt: Double) = {
    ...
    //println(bodies.toSeq)
  }

}


object AffineJovianSystem {
  ...
  class Body(){
    ...
    def speedSq: AffineFloat = vx*vx + vy*vy + vz*vz
  ...
  private def initialValues() = {
    val SOLAR_MASS: AffineFloat = 4 * Pi * Pi
    val DAYS_PER_YEAR = AffineFloat(365.24)
    ...
    var px = AffineFloat(0.0); var py = AffineFloat(0.0); var pz = AffineFloat(0.0);
    for (b <- initialValues){
       px += (b.vx * b.mass)
       py += (b.vy * b.mass)
       pz += (b.vz * b.mass)
...
object IntervalJovianSystem {
  ...
  class Body(){
    ...
    def speedSq: IntervalFloat = vx*vx + vy*vy + vz*vz
  ...
  private def initialValues() = {
    val SOLAR_MASS: IntervalFloat = 4 * Pi * Pi
    val DAYS_PER_YEAR = IntervalFloat(365.24)
    ...
    var px = IntervalFloat(0.0); var py = IntervalFloat(0.0); var pz = IntervalFloat(0.0);
    for (b <- initialValues){
       px += (b.vx * b.mass)
       py += (b.vy * b.mass)
       pz += (b.vz * b.mass)
...
object SmartJovianSystem {
  ...
  class Body(){
    ...
    def speedSq: SmartFloat = vx*vx + vy*vy + vz*vz
  ...
  private def initialValues() = {
    val SOLAR_MASS: SmartFloat = 4 * Pi * Pi
    val DAYS_PER_YEAR = SmartFloat(365.24)
    ...
    var px = SmartFloat(0.0); var py = SmartFloat(0.0); var pz = SmartFloat(0.0);
    for (b <- initialValues){
       px += (b.vx * b.mass)
       py += (b.vy * b.mass)
       pz += (b.vz * b.mass)
