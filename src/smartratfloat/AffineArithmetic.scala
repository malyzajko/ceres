...
object AffineArithmetic {
  ...
  case class Value(d: Double, q: Queue, r: Rational) extends Result
  ...
  def plus(x0: Double, xnoise: Queue, y0: Double, ynoise: Queue): Result = {
    ...
    var roundoff = if (addD(x0, y0) == addU(x0, y0)) zero
      else Rational(DirRound.roundOff(z0))

    var deviation = addQueuesUncertain(xnoise, ynoise)
    
    if(roundoff.toDouble == PlusInf) return All
    if (roundoff != zero) deviation :+ new Noise(newIndex, roundoff)
  ...
  def multiply(x0: Double, xnoise: Queue, xRadius: Rational,
            y0: Double, ynoise: Queue, yRadius: Rational, hint: Double = NaN): Result = {
    ...
    var deviation = multiplyQueuesUncertain(x0, xnoise, y0, ynoise)
    //delta = addUp(delta, deltaAddition)

    if(delta.toDouble == PlusInf) return All
    if (delta != zero) deviation :+ new Noise(newIndex, delta)
    return Value(z0, deviation, roundoff)
  }
