...
case class CenterForm(x0: Double, var xnoise: Queue) extends AbstractAForm {
  ...
  def /(y: AffineForm): AffineForm = y match {
    case CenterForm(y0, ynoise) =>
      ...
      val (ylo: Rational, yhi: Rational) = y.intervalExt
...
/*
  ...
  */
  ...
  private def symmetricUnary(z0: Double, xnoise: Queue, alpha: Rational, zeta: Rational, delta: Rational) : AffineForm = {
    //unaryCount +=1
    var d = delta
    ...
    while(iter.hasNext) {
      ...
      val zi =  alpha * xiValue
      //d =  addUp(d,  rdOff(zi))
      if(zi != zero) deviation :+ new Noise(xi.index, zi)
    }
    if(d.toDouble == PlusInf) return FullForm
    if(d != zero) deviation :+ new Noise(newIndex, d)
